...
public class MedicinePanelShortCutKey {
    ...
    private final static Color BG_HOVER_COLOR = SystemColor.BG_HOVER_COLOR;
    private final static Color BG__COLOR = SystemColor.BG__COLOR;
    static Home home;
...
//        p.evening_chk.setForeground(BG__COLOR);     
    ...
    public static void getCurrentClicked(MedicineRowPanel p) {

        Database db = Database.getInstance();

        JLabel name = p.medicine_name;

        JPopupMenu popupMenu = new JPopupMenu();

        JMenuItem category = new JMenu("Category");
        popupMenu.add(category);

        JMenuItem bookmark = new JMenu("Bookmark");
        popupMenu.add(bookmark);

        ArrayList<String> bookmarkList = db.getBookmark();
        for (String b : bookmarkList) {
            JMenuItem b1 = new JMenuItem(b);
            bookmark.add(b1);

            b1.addActionListener((e) -> {

                if (db.addBookmark(b, p.getDetials().getMedicineDetails())) {
                    Notifications.getInstance().show(Notifications.Type.SUCCESS, Notifications.Location.TOP_RIGHT, "Medcine Added Successfully");
                } else {
                    Notifications.getInstance().show(Notifications.Type.SUCCESS, Notifications.Location.TOP_RIGHT, "Medcine Added Successfully");
        ...
        ArrayList<String> categoryList = db.getCategory();
        for (String cat : categoryList) {
            JMenuItem c1 = new JMenuItem(cat);
            category.add(c1);

            c1.addActionListener((e) -> {
                db.addCategory(c1.getText(), db.getMedicineNo(name.getText()));
                Notifications.getInstance().show(Notifications.Type.SUCCESS, Notifications.Location.TOP_RIGHT, "Medcine Added Successfully");
